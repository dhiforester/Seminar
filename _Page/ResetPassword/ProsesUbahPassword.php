    ...
    if(empty($_POST['email'])){
        ...
        if(empty($_POST['token'])){
            ...
            if(empty($DataAkses["id_akses"])){
                ...
                $QryLupaPassword=mysqli_query($Conn,"SELECT*FROM lupa_password WHERE id_akses='$id_akses' AND code_unik='$code_unik'")or die(mysqli_error($Conn));
                $DataLupaPassword = mysqli_fetch_array($QryLupaPassword);
                if(empty($DataLupaPassword["id_akses"])){
                                ...
                                if($JumlahKarakterPassword>20||$JumlahKarakterPassword<6||!preg_match("/^[a-zA-Z0-9]*$/", $_POST['PasswordBaru1'])){
                                    ...
                                    if($_POST['PasswordBaru1']!==$_POST['PasswordBaru2']){
                                        ...
                                        if($UpdateAkses){
                                            ...
                                            $query = mysqli_query($Conn, "DELETE FROM lupa_password WHERE id_akses='$id_akses'") or die(mysqli_error($Conn));
                                            if($query) {
