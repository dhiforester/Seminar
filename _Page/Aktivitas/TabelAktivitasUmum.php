<?php
    ...
    if(empty($keyword_by)){
        if(empty($keyword)){
            ...
            $jml_data = mysqli_num_rows(mysqli_query($Conn, "SELECT*FROM log WHERE id_akses like '%$keyword%' OR id_unit_kerja like '%$keyword%' OR datetime_log like '%$keyword%' OR kategori_log like '%$keyword%' OR deskripsi_log like '%$keyword%'"));
        }
    }else{
        if(empty($keyword)){
            $jml_data = mysqli_num_rows(mysqli_query($Conn, "SELECT*FROM log"));
...
<script>
    ...
    $('#NextPage').click(function() {
        ...
        $.ajax({
            ...
            method  : "POST",
            data 	:  { page: valueNext, BatasAktivitasUmum: batas, keyword: keyword, keyword_by: keyword_by, OrderBy: OrderBy, ShortBy: ShortBy },
    ...
    $('#PrevPage').click(function() {
        ...
        $.ajax({
            url     : "_Page/Aktivitas/TabelAktivitasUmum.php",
            method  : "POST",
            data 	:  { page: ValuePrev, BatasAktivitasUmum: batas, keyword: keyword, keyword_by: keyword_by, OrderBy: OrderBy, ShortBy: ShortBy },
            success : function (data) {
                $('#MenampilkanTabelAktivitasUmum').html(data);
            }
        })
    ...
    ?>
        ...
        $('#PageNumber<?php echo $i;?>').click(function() {
            ...
            $.ajax({
                url     : "_Page/Aktivitas/TabelAktivitasUmum.php",
                method  : "POST",
                data 	:  { page: PageNumber, BatasAktivitasUmum: batas, keyword: keyword, keyword_by: keyword_by, OrderBy: OrderBy, ShortBy: ShortBy },
                success: function (data) {
                    $('#MenampilkanTabelAktivitasUmum').html(data);
                }
            })
                ...
                <table class="table table-hover table-bordered align-items-center mb-0">
                    ...
                    <tbody>
                            ...
                            if(empty($jml_data)){
                                ...
                                if(empty($keyword_by)){
                                    if(empty($keyword)){
                                        ...
                                        $query = mysqli_query($Conn, "SELECT*FROM log WHERE id_akses like '%$keyword%' OR id_unit_kerja like '%$keyword%' OR datetime_log like '%$keyword%' OR kategori_log like '%$keyword%' OR deskripsi_log like '%$keyword%' ORDER BY $OrderBy $ShortBy LIMIT $posisi, $batas");
                                    }
                                }else{
                                    if(empty($keyword)){
                                        $query = mysqli_query($Conn, "SELECT*FROM log ORDER BY $OrderBy $ShortBy LIMIT $posisi, $batas");
                                ...
                                while ($data = mysqli_fetch_array($query)) {
                                    ...
                                    //Apabila id_unit_kerja tidak kosong
                                    if(!empty($data['id_unit_kerja'])){
                                        ...
                                        $nama_unit_kerja="<span class='text-info'>$nama_unit_kerja</span>";
                                    }else{
                                        $nama_unit_kerja="<span class='text-danger'>General</span>";
                                    }
                        ...
                        <tr>
                            ...
                            <td class="text-left" align="left">
                                <small><?php echo "<b>$nama_akses</b>" ?></small>
                            </td>
                            <td class="text-left" align="left">
                                <?php 
