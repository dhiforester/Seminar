<?php
    ...
    if(empty($_POST['id_setting_event'])){
        ...
        //Hitung Jumlah Data
        $jml_data = mysqli_num_rows(mysqli_query($Conn, "SELECT*FROM event_pengisi_acara WHERE id_event_setting='$id_event_setting'"));
?>
    <input type="hidden" name="PutIdEvent" value="<?php echo "$id_event_setting"; ?>">
    <input type="hidden" name="kategori_sertifikat" value="Pengisi Acara">
    <div class="row">
        <div class="col-md-12 text-center mb-3">
            <span><?php echo "Daftar Pengisi Acara <br><code class='text-primary'>$nama_event</code>";?></span>
        </div>
    </div>
    <div class="row">
        <?php
            if(empty($jml_data)){
                ...
                $query = mysqli_query($Conn, "SELECT*FROM event_pengisi_acara WHERE id_event_setting='$id_event_setting' ORDER BY id_event_pengisi_acara ASC");
                while ($data = mysqli_fetch_array($query)) {
                    $id_event_pengisi_acara= $data['id_event_pengisi_acara'];
                    $nama= $data['nama'];
                    //Buka Token
                    $QrySertifikat= mysqli_query($Conn,"SELECT * FROM event_sertifikat WHERE id_person='$id_event_pengisi_acara' AND kategori_sertifikat='Pengisi Acara'")or die(mysqli_error($Conn));
        ...
        ?>
            <li class="list-group-item d-flex justify-content-between align-items-start">
                <div class="ms-2 me-auto">
                    <label for="CheckBoxPengisiAcara<?php echo $id_event_pengisi_acara;?>">
                ...
                <input class="form-check-input me-1" type="checkbox" value="<?php echo $id_event_pengisi_acara;?>" id="CheckBoxPengisiAcara<?php echo $id_event_pengisi_acara;?>" name="CheckIdPerson[]">
            </li>
        <?php
                $no++; }
                echo '  </ol>';
        ...
        <div class="col-md-12 mb-3">
            <small id="NotifikasiGenerateTokenPengisiAcara">
                <code class="text-primary">Silahkan checklist data pengisi acara yang akan dibuatkan token sertifikatnya.</code>
            </small>
        </div>
